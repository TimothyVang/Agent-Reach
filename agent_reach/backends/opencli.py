...
def opencli_status(timeout: int = 10) -> OpenCLIStatus:
    ...
    if not version_probe.ok:
        return OpenCLIStatus(
            ...
            broken=True,
            hint=(
                "opencli command exists but cannot run (broken node environment); reinstall:\n"
...
def opencli_summary(st: OpenCLIStatus) -> str:
    ...
    if st.daemon_running:
        return "OpenCLI installed, waiting for Chrome extension to be installed"
    return "OpenCLI installed (daemon not running, starts automatically on use; needs Chrome extension)"
